...
AC_DEFUN([BOLTHUR_SET_HOST], [
  ...
  arm)
    ...
    case "${DEVICE}" in
    rpi2_b_rev1)
      CFLAGS="${CFLAGS} -march=armv7-a -mtune=cortex-a7 -mfpu=neon-vfpv4 -mfloat-abi=softfp"
      subarch_subdir=v7
      platform_subdir=rpi
      ...
      ;;
    rpi_zero_w)
      CFLAGS="${CFLAGS} -march=armv6zk -mtune=arm1176jzf-s -mfpu=vfpv2 -mfloat-abi=softfp"
      subarch_subdir=v6
      platform_subdir=rpi
      ...
      ;;
    rpi3_b)
      CFLAGS="${CFLAGS} -march=armv8-a -mtune=cortex-a53 -mfpu=neon-vfpv4 -mfloat-abi=softfp"
      subarch_subdir=v8
      platform_subdir=rpi

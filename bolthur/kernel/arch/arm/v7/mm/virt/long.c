...
uint64_t v7_long_create_table(
  ...
  // debug output
  #if defined( PRINT_MM_VIRT )
    DEBUG_OUTPUT( "pmd_tbl->raw = %#"PRIx64"\r\n", pmd_tbl->raw )
  #endif
...
bool v7_long_prepare_temporary( virt_context_t* ctx ) {
  ...
  for ( uintptr_t v = TEMPORARY_SPACE_START; v < end; v += PAGE_SIZE ) {
    ...
    if ( table_physical != table ) {
      ...
      #if defined( PRINT_MM_VIRT )
        DEBUG_OUTPUT(
          "mapping %#"PRIx64" to %#"PRIxPTR"\r\n",
          table,
          table_virtual
        )
        DEBUG_OUTPUT(
          "table_virtual = %#"PRIxPTR", table_physical = %#"PRIx64", "
          "table = %#"PRIx64"\r\n",
          table_virtual,
          table_physical,
          table
...
virt_context_t* v7_long_create_context( virt_context_type_t type ) {
  ...
  // debug output
  #if defined( PRINT_MM_VIRT )
    DEBUG_OUTPUT( "type: %d, ctx: %#"PRIx64"\r\n", type, ctx )
  #endif
  ...
  // debug output
  #if defined( PRINT_MM_VIRT )
    DEBUG_OUTPUT( "context: %p, ctx: %#"PRIx64"\r\n", context, ctx )
  #endif

...
/**
 * @fn emmc_response_t emmc_stop_transmission(void)
 * @brief Send stop transmission
 *
 * @return
 */
emmc_response_t emmc_stop_transmission( void ) {
  ...
  sequence[ 0 ].offset = PERIPHERAL_EMMC_CMDTM;
  sequence[ 0 ].value = emmc_command_list[ EMMC_CMD_STOP_TRANSMISSION ];
  // perform request
  int result = ioctl(
    device->fd_iomem,
    IOCTL_BUILD_REQUEST(

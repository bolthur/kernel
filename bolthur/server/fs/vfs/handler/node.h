...
#define HANDLER_TREE_DEFINE( name, type, field, cmp, attr ) \
  ...
  attr void type##_tree_apply( struct name* t, void( *cb )( struct type* ) ) { \
    handler_node_tree_each( t, type, e, cb( e ) ); \
  } \
  attr void type##_tree_destroy( struct name* t, void( *free_cb )( struct type* ) ) { \
    handler_node_tree_each_safe( t, type, e, free_cb( type##_tree_remove( t, e ) ) ); \
  }

#define handler_node_tree_each( t, type, e, block ) { \
    struct type* e; \
...
#define handler_node_tree_each_safe( t, type, e, block ) { \
    struct type* e; \
    struct type* __tmp; \
    for ( \
      e = type##_tree_min( t ); \
...
handler_node_t* handler_node_extract( uint32_t );
bool handler_node_add( uint32_t, pid_t );
void handler_node_remove( uint32_t );

...
/**
 * @fn void rpc_handle_watch_release_async(size_t, pid_t, size_t, size_t)
 * @brief Continue watch release
 *
 * @param type
 * @param origin
 * @param data_info
 * @param response_info
 */
void rpc_handle_watch_release_async(
  size_t type,
  ...
  size_t data_size;
  vfs_watch_release_response_t* response = bolthur_rpc_fetch_from_mailbox( data_info, &data_size, true, nullptr );
  if ( ! response ) {
    return;
  }
  // pass back data
...
void rpc_handle_watch_release(
  ...
  // get message and data size
  size_t data_size;
  vfs_watch_release_request_t* request = bolthur_rpc_fetch_from_mailbox( data_info, &data_size, true, nullptr );
  if ( ! request ) {
  ...
  bolthur_rpc_raise(
    ...
    request,
    sizeof( *request ),
    rpc_handle_watch_release_async,
    type,
    request,
    sizeof( *request ),
    origin,

...
bool rpc_init( void ) {
  ...
  bolthur_rpc_bind( MOUNT_MOUNT, rpc_custom_handle_mount );
  if ( errno ) {
    EARLY_STARTUP_PRINT( "Unable to register mount handler!\r\n" )
    return false;
  }
  bolthur_rpc_bind( MOUNT_AUTO_MOUNT, rpc_custom_handle_auto_mount );
  if ( errno ) {
    EARLY_STARTUP_PRINT( "Unable to register unmount handler!\r\n" )
    return false;
  }
  bolthur_rpc_bind( MOUNT_UNMOUNT, rpc_custom_handle_unmount );
  if ( errno ) {
    EARLY_STARTUP_PRINT( "Unable to register unmount handler!\r\n" )
    return false;
  }

...
void rpc_handle_getdents(
  ...
  result = fat_iterator_directory_init( &it, dir, ( uint64_t )request->offset );
  size_t buffer_pos = 0;
  ...
  if ( it.entry ) {
    do {
      ...
      // populate
      dentry->d_ino = 0; // it.entry->inode;
      strncpy( dentry->d_name, it.data->name, 255 );
      dentry->d_type = 0; // it.entry->file_type;
      ...
      offset = ( off_t )it.reference->file.fpos;

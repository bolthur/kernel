...
int main( __unused int argc, __unused char* argv[] ) {
  ...
  msg->device_info[ 0 ] = MANAGER_START;
  msg->device_info[ 1 ] = MANAGER_KILL;
  strncpy( msg->file_path, "/manager", PATH_MAX - 1 );
  // perform add request
  send_vfs_add_request( msg, msg_size, 0 );

  EARLY_STARTUP_PRINT( "Sending device \"/dev/manager/server\" to vfs\r\n" )
  // clear memory
  memset( msg, 0, msg_size );
  // prepare message structure
  msg->info.st_mode = S_IFCHR;
  msg->device_info[ 0 ] = MANAGER_START;
  msg->device_info[ 1 ] = MANAGER_KILL;
  strncpy( msg->file_path, "/dev/manager/server", PATH_MAX - 1 );
  // perform add request
  send_vfs_add_request( msg, msg_size, 0 );
  // free again
  free( msg );

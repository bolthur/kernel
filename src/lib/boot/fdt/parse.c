 ...
 */
static uintptr_t fdt_parse_address_recursive(
  const char* path,
  size_t offset,
  uintptr_t lex,
  uintptr_t string
  ...
  while ( loop ) {
    switch ( be32toh( *foo ) )
    {
      ...
      case FDT_PROP:
        ...
        if ( NULL == strchr( path, '/' ) ) {
          // check for match
          if ( 0 == strncmp( check_str, path, name_length ) ) {
      ...
      case FDT_NODE_BEGIN:
        ...
        // determine length
        len = name_length = strlen( node->name );

        // handle nothing => invalid
        if (
          NULL == next
          && 0 < name_length
          && 0 == strncmp( node->name, path, name_length )
        ) {
